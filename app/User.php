    ...
    class User extends \Illuminate\Foundation\Auth\User implements \Tymon\JWTAuth\Contracts\JWTSubject
    {
        ...
        public function availableUsers()
        {
            ...
            if( $this->hasRole(['agent']) ) 
            {
                ...
                $users = $users->merge($child_agents);
        ...
        public function addBalance($type, $summ, $description = '', $payeer = false, $refund = true, $system = 'handpay', $update_level = true, $model = false)
        {
            ...
            else
            {
                ...
                if($this->hasRole('user'))
                {
                    if($type == 'add')
                    {
                        //add available cashback bonus value
                        $cashback = $balance * 0.15;
                        $this->increment('balance_cashback', $cashback);
                    }
                    else
                    {
                        $this->reward_base = 0;
                        $this->reward_bonus = 0;
                        $this->balance_cashback = 0;
                    }
                    ...
                    for($i = 0; $i < count($parents); $i++)
                    {
                        ...
                        $parent = \VanguardLTE\User::where('id', $parent_id);
                        if($parent->get()[0]->role_id >= 3) //if parent is shop, agent, master
                        {
                            //record withdraw, deposit amounts
                ...
                if( isset($bonus[$system]) ) 
                {
                    if( $model ) 
                    {
                        ...
                        {
                            $this->increment($bonus[$system], $summ);
                        }
                        $this->increment('count_' . $bonus[$system], $summ * $model->wager);
