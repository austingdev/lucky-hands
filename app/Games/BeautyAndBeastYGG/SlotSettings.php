...
namespace VanguardLTE\Games\BeautyAndBeastYGG
{
    ...
    class SlotSettings extends BasicSlotSettings
    {
        ...
        public function GetReelStrips($winType, $slotEvent)
        {
            ...
            {
                $reel = new GameReel(); 
                if( $slotEvent == 'freespin_chef' ) 
                    $fArr = $reel->reelsStripBonus;
                else if($slotEvent == 'freespin_butler')
                    $fArr = $reel->reelsStripBonusC;
                else if($slotEvent == 'freespin_maid')
                    $fArr = $reel->reelsStripBonusB;
                else if($slotEvent == 'boost')
                    $fArr = $reel->reelsStripSuperbet1;
                else if($slotEvent == 'nudge')
                    $fArr = $reel->reelsStripSuperbet2;
                else if($slotEvent == 'boost_nudge')
                    $fArr = $reel->reelsStripSuperbet3;
            ...
            foreach( $arrReels as $index => $reelStrip ) 
            {
                if( is_array($this->$reelStrip) && count($this->$reelStrip) > 0 ) 
                {
                    if($winType == 'win')
                    {
                        $prs[$index + 1] =$this->GetRandomScatterPos($this->$reelStrip, rand(1,3));
                    }
                    else
            ...
            if($slotEvent == 'bet' && $winType == 'bonus')
            {
                $reelsId = [];
                foreach( $arrReels as $index => $reelStrip ) 
                {
                    if( is_array($this->$reelStrip) && count($this->$reelStrip) > 0 ) 
                    {
                        $prs[$index + 1] = $this->GetRandomScatterPos($this->$reelStrip, 0);
                        $reelsId[] = $index + 1;
                    }
                }
        ...
        public function GetNoWinSpin($slotEvent, $gameMode = 0, $wildseed = [])
        {
            $isWin = true;
            ...
            while($isWin)
            {
                ...
                // else if($gameMode == 6)
                // {
                //     for($w = 0; $w < count($wildseed); $w++)
                //     {
                //         $seedPosition = $wildseed[$w];
        ...
        public function GetNudgedReel($reelIndex, $rp, $nudge)
        {
            $nudgeReel = new GameReel();
            $arr = $nudgeReel->reelsStripBonusC['reelStripBonusC'.($reelIndex+1)];
            if($nudge == -1 && $rp < 3)
                return [];
            if($nudge == 1 && $rp > count($arr) - 4)
                return [];
            $res = array_slice($arr, ($rp + $nudge) - 1, 3);
