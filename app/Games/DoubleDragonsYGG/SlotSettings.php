...
namespace VanguardLTE\Games\DoubleDragonsYGG
{
    ...
    class SlotSettings extends BasicSlotSettings
    {
        ...
        public function __construct($sid, $playerId)
        {
            ...
            $this->Paytable['WILD_ICEHEAD'] = [0, 0, 0, 0, 0, 0];
            $this->Paytable['WILD_ICE'] = [0, 0, 0, 0, 0, 0];
            $this->Paytable['WILD_FIREHEAD'] = [0, 0, 0, 0, 0, 0];
            $this->Paytable['WILD_FIRE'] = [0, 0, 0, 0, 0, 0];
            $this->Paytable['High1'] = [0, 0, 0, 20, 60, 125];
            $this->Paytable['High2'] = [0, 0, 0, 15, 50, 100];
            $this->Paytable['High3'] = [0, 0, 0, 10, 40, 90];
            $this->Paytable['High4'] = [0, 0, 0, 10, 40, 80];
            $this->Paytable['Low1'] = [0, 0, 0, 5, 25, 50];
            $this->Paytable['Low2'] = [0, 0, 0, 5, 25, 50];
            $this->Paytable['Low3'] = [0, 0, 0, 4, 20, 40];
            $this->Paytable['Low4'] = [0, 0, 0, 4, 20, 40];
        ...
        public function GetReelStrips($winType, $reelName, $lastReel)        
        {
            $filename = base_path() . '/app/Games/DoubleDragonsYGG/'.$reelName.'.txt';
            ...
            if($winType != 'bonus')
            {
                if($winType == 'win')
                {
                    ...
                    {
                        $sym = 0;
                        $syms = ["High1","High2","High3","High4","Low1","Low2","Low3","Low4"];
                        $sym = $syms[rand(0, 7)];
                        for($i = 0; $i < 5; $i++)
                        {
                            $position = -1;
                            ...
                            $prs[$i + 1] = $this->GetSymbolPos($reelStrips->reels[$i], $sym, 4, $position);
                        }
                    }
                    else
                    {
                        ...
                        for($i = 0; $i < 5; $i++) 
                        {
                            $prs[$i + 1] = $this->GetSymbolPos($reelStrips->reels[$i], $sym, 4);
                        }
                    }
                }
                else
                {
                    for($i = 0; $i < 5; $i++) 
                    {
                        $prs[$i + 1] = mt_rand(1, count($reelStrips->reels[$i]) - $endLen);
                    }
                }                
            }
            else
            {
                $symb = rand(0, 100) > 50 ? 'WILD_ICEHEAD' : 'WILD_FIREHEAD';
                // $symb = 'WILD_ICEHEAD';
                $reelsId = [];
                foreach($reelStrips->reels as $index => $reelStrip ) 
                {
                    $reelsId[] = $index + 1;
                    $prs[$index + 1] = 0;
                }
                ...
                for( $i = 0; $i < count($reelsId); $i++ ) 
                {
                    if( $i < $scattersCnt ) 
                    {
                        $prs[$reelsId[$i]] = $this->GetRandomScatterPos($reelStrips->reels[$reelsId[$i] - 1], $endLen, $symb);
                    }
                    else
                    {
                        $prs[$reelsId[$i]] = rand(1, count($reelStrips->reels[$reelsId[$i] - 1]) - $endLen);
                    }
                }
            }
        ...
        function getMultiplier($reel, $sym, $boardHeight)
        {
            $wild = 'WILD';
            $multiplier = 0;
