<?php 
namespace VanguardLTE\Games\BuffaloThunder
{
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId)
        {
            ...
            $this->hideButtons = [];
            $this->jpgs = \VanguardLTE\JPG::where('shop_id', -1)->lockForUpdate()->get();
        ...
        public function SetBalance($sum, $slotEvent = '')
        {
            ...
            {
                $user->cashable_balance = $user->cashable_balance + $sum;
                $user->cashable_balance = $this->FormatFloat($user->cashable_balance);
            }
            $this->user->session = serialize($this->gameData);
            $this->user->save();
            $this->user->refresh();
        ...
        public function GetBalance()
        {
            if (\Auth::check())
            {
                ...
                $this->shop_id = $user->shop_id;
                $this->Balance = $user->balance;
                $this->CashableBalance = $user->cashable_balance;
                $this->shop = \VanguardLTE\Shop::find($this->shop_id);
                $this->Percent = $this->shop->percent;
                $this->jpgs = \VanguardLTE\JPG::where('shop_id', -1)->lockForUpdate()->get();
                $this->slotCurrency = $user->shop->currency;
