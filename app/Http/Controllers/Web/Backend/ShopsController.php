...
namespace VanguardLTE\Http\Controllers\Web\Backend
{
    ...
    class ShopsController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function store(\Illuminate\Http\Request $request)
        {
            ...
            \DB::statement('insert '.\DB::getTablePrefix().$jpg_table.'(name,balance,start_balance,pay_sum,percent,shop_id, start_payout, end_payout) (select name,balance,start_balance,pay_sum,percent, '.$shop->id.' as shop_id, start_payout, end_payout from '.\DB::getTablePrefix().$jpg_table.' where shop_id=0)');
        ...
        public function edit($shop)
        {
            ...
            //get parents
            $user = \VanguardLTE\User::find($shop->user_id);
            ...
            $jackpots = \VanguardLTE\JPG::where('shop_id', '=', $user->shop_id)->get();
            return view('backend.shops.edit', compact('shop', 'directories', 'categories', 'cats', 'blocks', 'activity', 'selector', 'contact_info', 'user', 'zones', 'selectedZone', 'jackpots'));
        }
        ...
        public function update(\Illuminate\Http\Request $request, \VanguardLTE\Repositories\Session\SessionRepository $sessionRepository, \VanguardLTE\Shop $shop)
        {
            ...
            $shop->update(['percent' => $req_data['rtp'], 'rtp_type' => $req_data['rtp_type']]);
            ...
            //update jackpot
            if(isset($req_data['jackpots_status']))
            {
                $jackpot_status = $req_data['jackpots_status'];
                \VanguardLTE\Shop::where('id', '=', $shop->id)->update(['jackpot_active' => $jackpot_status == 'enabled' ? 1 : 0]);

                $jackpot_fee = $req_data['jp_fee'];
                $jackpot_fake_count = $req_data['jp_fake_count'];
                \VanguardLTE\JPG::where('shop_id', '=', $shop->id)->update(['fake_cnt' => $jackpot_fake_count, 'percent' => $jackpot_fee, 'tick' => 0]);            
            }
            //update new games
            $total_games = \VanguardLTE\Game::where('shop_id', 0)->get();
            $shop_games = \VanguardLTE\Game::where('shop_id', $shop->id)->pluck('name')->toArray();
            foreach($total_games as $game)
            {
                if(!in_array($game->name, $shop_games))
                {
                    $new_shop_game = $game->replicate();
                    $new_shop_game->shop_id = $shop->id;
                    $new_shop_game->save();
                }
            }
            return redirect()->route('backend.user.list')->withSuccess(trans('app.shop_updated'));
        }
        ...
        {
            $data = $request->all();
            $request->shop_id = $data['DepositeForm']['id'];
            $request->summ = $data['DepositeForm']['amount'];            
            $request->type = $data['DepositeForm']['type'];
            
            $shop = \VanguardLTE\Shop::find($request->shop_id);
            $user = \VanguardLTE\User::find($shop->user_id);
            $user = \VanguardLTE\User::find($user->parent_id); //payeer
            if( $request->all && $request->all == '1' ) 
            {
                $request->summ = $shop->balance;
            }
            $summ = abs(floatval($request->summ));
            if( !$shop ) 
            {
                abort(404);
            }
            ...
            {
                if( $request->type == 'add' && $user->balance < $summ ) 
                {
            ...
            if( $request->type == 'out' && $shop->balance < $summ ) 
            {
            ...
            $shop_manager = \VanguardLTE\User::where('shop_id', '=', $request->shop_id)
                              ->where('role_id', '=', 3)->get(['id'])[0];
            
            \VanguardLTE\Statistic::create([
                'user_id' => $shop_manager->id,
                'payeer_id' => auth()->user()->id,
                'shop_id' => $shop->id,
                'last_balance' =>  $shop->balance,
                'result_balance' => $shop->balance + $sum,
                'last_payeer_balance' => $user->balance,
                'result_payeer_balance' => $user->balance - $sum,
                'type' => $request->type,
                'description' => $request->type == 'add' ? 'Deposit' : 'Reedem',
                'sum' => abs($sum),
                'system' => 'shop'
            ]);
        ...
        public function regenerate_jp(\Illuminate\Http\Request $request, \VanguardLTE\Shop $shop)        
        {
            $data = $request->all();
            ...
            JPG::where('name', '=', $jackpot_name)->where('shop_id', '=', $shop->id)->update([
                'pay_sum' => $pay_sum,
        ...
        {
            $danger_shops = DB::select(DB::raw("select A.*, B.name from w_game_bank A
            inner join w_shops B on A.shop_id = B.id
            where slots < 0 or bonus < 0"));
            return view('backend.shops.danger', compact('danger_shops'));
        }
    }
